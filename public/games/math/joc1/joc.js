var cg_aanSchimbariMinMax;
var g_nSubtip = 0;
var g_nDificultate;
var g_nNumarCuvinte;
var g_nNumarCuvinteSchimbate;
var g_nStare;
var g_nIndexPaginaInstructiuni;
var g_nIndexCuvint;
var g_nCuvinteCorecte;
var g_bGreseala;
...
var g_asCuvinte = new Array();
var g_acCuvinte = new Array();
...
function Init()
{
	...
	cg_aanSchimbariMinMax = new Array();
	for(i=0; i<g_nNumarExercitii; i++) cg_aanSchimbariMinMax[i] = new Array(Math.ceil((i + 1) / 2), i + 1);
...
function StartJoc()
{
	...
	var abCuvinteSelectate = new Array();
	var sText;
	var nRandom;
	var i, j;
	...
	g_nNumarCuvinte = g_nDificultate * 4;
	for (i=0; i<g_nNumarCuvinte; i++) g_acCuvinte[i] = new Cuvint("");
	...
	for(i=0; i<87; i++) abCuvinteSelectate[i] = false;

	for(i=0; i<10; i++)
	{
		nRandom = Random(0, 86 - i);
		for(j=0; j<87; j++)
			if (abCuvinteSelectate[i] == false)
				if (nRandom == 0)
				{
					g_asCuvinte[i] = cg_asCuvinte[j];
					break;
				}
				else nRandom--;
	}
...
{
	var anDeltaXLinie = new Array();
	var abChenarOcupat = new Array();
	var nMin, nMax, nRandom, nLiteraSchimbata, nLiteraNoua, nLiteraVeche, nChenareLibere, nLatimeCuvint, nDeltaX, nDeltaY, nLinie, nColoana;
	var i, j;

	for(i=0; i<g_nNumarCuvinte; i++)
	{
		g_acCuvinte[i].sText = g_asCuvinte[g_nIndexCuvint];
		g_acCuvinte[i].bSchimbat = false;
		g_acCuvinte[i].bApasat = false;
	}

	nMin = cg_aanSchimbariMinMax[g_nDificultate-1][0];
	nMax = cg_aanSchimbariMinMax[g_nDificultate-1][1];

	g_nNumarCuvinteSchimbate = Random(nMin, nMax);

	for(i=0; i<g_nNumarCuvinteSchimbate; i++)
	{
		nRandom = Random(0, g_nNumarCuvinte - i - 1);
		for(j=0; j<g_nNumarCuvinte; j++)
			if (g_acCuvinte[j].bSchimbat == false)
				if (nRandom == 0)
				{
					g_acCuvinte[j].bSchimbat = true;
					nLiteraSchimbata = Random(0, g_acCuvinte[j].sText.length - 1);
					nLiteraVeche = g_acCuvinte[j].sText.charCodeAt(nLiteraSchimbata);
					nLiteraNoua = Random(97, 121);
					if (nLiteraNoua >= nLiteraVeche) nLiteraNoua++;
					g_acCuvinte[j].sText = g_acCuvinte[j].sText.substr(0, nLiteraSchimbata) + String.fromCharCode(nLiteraNoua) + g_acCuvinte[j].sText.substr(nLiteraSchimbata + 1, g_acCuvinte[j].sText.length - nLiteraSchimbata - 1);
					break;
				}
				else nRandom--;
	}

	for(i=0; i<10; i++) anDeltaXLinie[i] = Random(0, 40);

	g_cContext.font = "18px Arial";
	nChenareLibere = 50;
	for(i=0; i<50; i++) abChenarOcupat[i] = false;
	for(i=0; i<g_nNumarCuvinte; i++)
	{
		...
		for(j=0; j<50; j++)
			if (abChenarOcupat[j] == false)
				if (nRandom == 0)
				{
					...
					nLatimeCuvint = g_cContext.measureText(g_acCuvinte[i].sText).width;
					nLinie = Math.floor(j / 5);
					nColoana = j % 5;
					nDeltaX = Random(1, 119 - nLatimeCuvint);
					nDeltaY = Random(2, 20);
					g_acCuvinte[i].x1 = anDeltaXLinie[nLinie] + 120 * nColoana + nDeltaX;
					g_acCuvinte[i].y1 = 40 * nLinie + nDeltaY;
					g_acCuvinte[i].x2 = g_acCuvinte[i].x1 + nLatimeCuvint;
					g_acCuvinte[i].y2 = g_acCuvinte[i].y1 + 18;
					break;
				}
				else nRandom--;
		nChenareLibere--;
...
function AfisareExercitiu()
{
	...
	for(i=0; i<g_nNumarCuvinte; i++) g_cContext.fillText(g_acCuvinte[i].sText, g_acCuvinte[i].x1, g_acCuvinte[i].y1 + 9);
}
...
function Urmatorul()
{
	...
	if (g_nIndexCuvint == g_nNumarExercitii - 1)
	{
		SchimbaStarea(4);
		AfisareRezultate();
...
function ProcesareRezultate()
{
	...
	sParam += ReprezentareHexa(1);   // ID joc
...
function MesajDeLaServer()
{
	if ((g_cXMLHTTP.readyState == 4) && (g_cXMLHTTP.status == 200))
	{
		...
		document.getElementById("debug").innerHTML += "<br>" + g_cXMLHTTP.responseText;
	}
}
			...
			case 3:	{
						if (g_cButonReset.Deasupra())
						{
							...
							for(i=0; i<g_nNumarCuvinte; i++)
								if (DeasupraCuvint(i))
								{
									if (g_acCuvinte[i].bApasat);
									else
									{
										g_acCuvinte[i].bApasat = true;
										if (g_acCuvinte[i].bSchimbat)
										{
											g_cContext.font = "18px Arial";
											g_cContext.textBaseline = "middle";
											g_cContext.textAlign = "left";
											g_cContext.fillStyle = "#00c000";
											g_cContext.fillText(g_acCuvinte[i].sText, g_acCuvinte[i].x1, g_acCuvinte[i].y1 + 9);
											RedareSunet(1);
											g_nCuvinteCorecte++;
											if (g_nCuvinteCorecte == g_nNumarCuvinteSchimbate)
											{
...
function DeseneazaMeniu()
{
	...
	sText = TEXT_CUVANTUL + " " + (g_nIndexCuvint + 1).toString() + " / " + g_nNumarExercitii.toString() + " :  ";
	g_cContext.fillText(sText, 10, 461);
	g_cContext.fillStyle = "#40c0f0";
	g_cContext.fillText(g_asCuvinte[g_nIndexCuvint], 10 + g_cContext.measureText(sText).width, 461);
...
function DeasupraCuvint(i)
{
	return (g_nMouseX > g_acCuvinte[i].x1) && (g_nMouseX < g_acCuvinte[i].x2) && (g_nMouseY > g_acCuvinte[i].y1 - 2) && (g_nMouseY < g_acCuvinte[i].y2 + 4);
}
...
function AfisareRezultate()
{
	...
	g_cContext.fillText(TEXT_DENUMIRE_JOC_001, 340, 30);
