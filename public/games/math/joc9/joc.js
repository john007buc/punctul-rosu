...
var g_anPachetCarti = new Array();
var g_anCartiMemorate = new Array();
var g_anCartiAfisate = new Array();
var g_nNumarCartiMemorate, g_nNumarCartiAfisate;
var g_nDeltaX;
...
var g_asFisiereImagini = new Array
								(
									...
									PATH + "imagini/carte-spate.png",
									PATH + "imagini/carti-fata.png"
								);
var IMG_MENIU = 3;
...
var IMG_CARTE_SPATE = 12;
var IMG_CARTI_FATA = 13;
...
function StartJoc()
{
	...
	g_nNumarCartiMemorate = g_nDificultate + 2;
	g_nNumarCartiAfisate = g_nNumarCartiMemorate * 2;
	g_nDeltaX = 120 + (5 - g_nNumarCartiMemorate) * 41;
...
function GenerareExercitiu()
{
	...
	{
		g_anPachetCarti[i] = 0;
		abPachetCarti[i] = false;
	}
	for(i=0; i<g_nNumarCartiMemorate; i++) g_anCartiMemorate[i] = -1;
	for(i=0; i<g_nNumarCartiAfisate; i++) g_anCartiAfisate[i] = -1;

	for(i=0; i<g_nNumarCartiMemorate; i++)
	{
		nRandom = Random(0, 51 - i);
		for(j=0; j<52; j++)
			if (g_anPachetCarti[j] == 1);
			else
				if (nRandom == 0)
				{
					g_anPachetCarti[j] = 1;
					abPachetCarti[j] = true;
					nRandom = Random(0, g_nNumarCartiMemorate - 1 - i);
					for(k=0; k<g_nNumarCartiMemorate; k++)
						if (g_anCartiMemorate[k] >= 0);
						else
							if (nRandom == 0)
							{
								g_anCartiMemorate[k] = j;
								break;
							}
							else nRandom--;
					nRandom = Random(0, g_nNumarCartiAfisate - 1 - i);
					for(k=0; k<g_nNumarCartiAfisate; k++)
						if (g_anCartiAfisate[k] >= 0);
						else
							if (nRandom == 0)
							{
								g_anCartiAfisate[k] = j;
								break;
							}
							else nRandom--;
					break;
	...
	for(i=0; i<g_nNumarCartiAfisate-g_nNumarCartiMemorate; i++)
	{
		nRandom = Random(0, 51 - i - g_nNumarCartiMemorate);
		for(j=0; j<52; j++)
			if (abPachetCarti[j]);
			else
				if (nRandom == 0)
				{
					abPachetCarti[j] = true;
					nRandom = Random(0, g_nNumarCartiAfisate - g_nNumarCartiMemorate - 1 - i);
					for(k=0; k<g_nNumarCartiAfisate; k++)
						if (g_anCartiAfisate[k] >= 0);
						else
							if (nRandom == 0)
...
function AfisareExercitiu()
{
	...
	for(i=0; i<g_nNumarCartiMemorate; i++) g_cContext.drawImage(g_acImagini[IMG_CARTI_FATA], (g_anCartiMemorate[i] % 13) * 73, Math.floor(g_anCartiMemorate[i] / 13) * 98, 72, 98, g_nDeltaX + i * 82, 52, 72, 98);
}
...
function AscundereExercitiu()
{
	var i, j;
	g_cContext.clearRect(0, 0, 640, 416);
	for(i=0; i<g_nNumarCartiMemorate; i++) AfisareImagine(IMG_CARTE_SPATE, g_nDeltaX + i * 82, 52);
	for(i=0; i<g_nNumarCartiMemorate; i++) g_cContext.drawImage(g_acImagini[IMG_CARTI_FATA], (g_anCartiAfisate[i] % 13) * 73, Math.floor(g_anCartiAfisate[i] / 13) * 98, 72, 98, g_nDeltaX + i * 82, 159, 72, 98);
	for(i=g_nNumarCartiMemorate; i<g_nNumarCartiAfisate; i++) g_cContext.drawImage(g_acImagini[IMG_CARTI_FATA], (g_anCartiAfisate[i] % 13) * 73, Math.floor(g_anCartiAfisate[i] / 13) * 98, 72, 98, g_nDeltaX + (i - g_nNumarCartiMemorate) * 82, 266, 72, 98);
}
...
function ProcesareRezultate()
{
	...
	sParam += ReprezentareHexa(9);   // ID joc
		...
		switch (g_nStare)
		{
			...
			case 3:	{
						if (g_cButonReset.Deasupra())
						{
							...
							if (g_nZecimiDeSecundaAfisareExercitiu > SECUNDE_AFISARE_EXERCITIU * 10)
								if ((g_nMouseX >= g_nDeltaX - 4) && (g_nMouseX <= g_nDeltaX + 82 * g_nNumarCartiMemorate - 5) && (g_nMouseY >= 155) && (g_nMouseY <= 368))
								{
									x = g_nMouseX - g_nDeltaX + 4;
									y = g_nMouseY - 155;
									nIndex = g_nNumarCartiMemorate * Math.floor(y / 107) + Math.floor(x / 82);

									if (g_anPachetCarti[g_anCartiAfisate[nIndex]] < 2)
										if (g_anPachetCarti[g_anCartiAfisate[nIndex]] == 1)
										{
											g_anPachetCarti[g_anCartiAfisate[nIndex]] = 2;
											RedareSunet(1);
											g_nCartiCorecte++;
											g_cContext.strokeStyle = "#00aa00";
											g_cContext.lineWidth = 3;
											g_cContext.strokeRect(g_nDeltaX - 1 + Math.floor(x / 82) * 82, 158 + Math.floor(y / 107) * 107, 74, 100);
											if (g_nCartiCorecte == g_nNumarCartiMemorate)
											{
...
function AfisareRezultate()
{
	...
	g_cContext.fillText(TEXT_DENUMIRE_JOC_009, 340, 30);
