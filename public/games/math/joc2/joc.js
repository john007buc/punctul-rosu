...
function StartJoc()
{
	...
	g_nNumarNumere = 9 * g_nDificultate;
	for (i=0; i<10 * g_nDificultate; i++) g_acNumere[i] = new Numar();
...
function GenerareExercitiu()
{
	...
	var nRandom, nChenareLibere, nLatimeNumar, nDeltaX, nDeltaY, nLinie, nColoana;
	var i, j;

	for(i=0; i<10; i++) anDeltaXLinie[i] = Random(8, 32);

	for(i=0; i<150; i++) abChenarOcupat[i] = false;

	for(i=0; i<10 * g_nDificultate; i++)
	{
		switch (Random(1, 3))
		{
			case 1:	{
						g_acNumere[i].nFont = 14;
						break;
					}
			case 2:	{
						g_acNumere[i].nFont = 18;
						break;
					}
			case 3:	{
						g_acNumere[i].nFont = 24;
						break;
					}
		}
	...
	for(i=0; i<g_nDificultate; i++)
	{
		nRandom = Random(0, 10 * g_nDificultate - i - 1);
		for(j=0; j<10*g_nDificultate; j++)
			...
			for(j=0; j<150; j++)
				if (abChenarOcupat[j] == false)
					if (nRandom == 0)
					{
						...
						g_cContext.font = g_acNumere[i].nFont.toString() + "px Arial";
						nLatimeNumar = g_cContext.measureText((i+1).toString()).width;
						nLinie = Math.floor(j / 15);
						nColoana = j % 15;
						if (nLatimeNumar > 38) nDeltaX = Math.floor(20 - (nLatimeNumar / 2));
						else nDeltaX = Random(1, 39 - nLatimeNumar);
						nDeltaY = Random(2, 38 - g_acNumere[i].nFont);
						g_acNumere[i].x1 = anDeltaXLinie[nLinie] + 40 * nColoana + nDeltaX;
						g_acNumere[i].y1 = 40 * nLinie + nDeltaY + 8;
						g_acNumere[i].x2 = g_acNumere[i].x1 + nLatimeNumar;
						g_acNumere[i].y2 = g_acNumere[i].y1 + 18;
						break;
					}
					else nRandom--;
			nChenareLibere--;
	...
	{
		g_nNumarCorect = 10 * g_nDificultate - 1;
		while (!g_acNumere[g_nNumarCorect].bPrezent) g_nNumarCorect--;
	}
}
...
function AfisareExercitiu()
{
	...
	for(i=0; i<10*g_nDificultate; i++)
		...
		{
			g_cContext.font = g_acNumere[i].nFont.toString() + "px Arial";
			g_cContext.fillText((i + 1).toString(), g_acNumere[i].x1 + Math.floor((g_acNumere[i].x2 - g_acNumere[i].x1) / 2), g_acNumere[i].y1 + Math.floor((g_acNumere[i].y2 - g_acNumere[i].y1) / 2));
		}
}
...
function ProcesareRezultate()
{
	...
	sParam += ReprezentareHexa(2);   // ID joc
									...
									if (DeasupraNumar(i))
									{
										...
										if (i == g_nNumarCorect)
										{
											...
											if (g_bCrescator)
												...
												{
													g_nNumarCorect++;
													if (g_nNumarCorect == 10 * g_nDificultate)
													{
...
function AfisareOptiuni()
{
	...
	if (g_bCrescator) AfisareImagine(IMG_CHECK, 109, 258);
	else AfisareImagine(IMG_CHECK, 109, 298);
}
...
{
	return (g_nMouseX > g_acNumere[i].x1 - 2) && (g_nMouseX < g_acNumere[i].x2 + 2) && (g_nMouseY > g_acNumere[i].y1 - 2) && (g_nMouseY < g_acNumere[i].y2 + 4);
}
...
function AfisareRezultate()
{
	...
	g_cContext.fillText(TEXT_DENUMIRE_JOC_002, 340, 30);
